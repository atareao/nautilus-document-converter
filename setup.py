...
DATA_FILES = [
('/usr/share/nautilus-python/extensions',glob.glob('src/*.py')),
('/usr/share/icons/Humanity/apps/48',['icons/nautilus_document_converter.png']),
]
...
def upload2ppa():
	...
	SOURCE_DIR = os.path.join(DIR,'src')
	DEBIAN_DIR = os.path.join(DIR,'debian')
	...
	if os.path.exists(CHANGELOG):
		print '#########################################################'		
		for filetorm in glob.glob(os.path.join(SOURCE_DIR,'*.pyc')):
			print 'removing %s'%filetorm
			os.remove(filetorm)
		f = open(CHANGELOG,'r')
		line = f.readline()
		f.close()
		...
		APP = line[:pos].strip()
		VERSION = line[pos+1:posf].strip()	
		TARFILE = APP + '_' + VERSION + '.orig.tar.gz'
		print '#########################################################'
		command = 'tar -c -v -z --exclude .bzr* -f %s ../%s/'%(os.path.join(PARENT_DIR,TARFILE),BASE_DIR)
		print command
		print ejecuta(command)
		print '#########################################################'
		command = 'debuild -S -sa -k%s'%KEY
		print command
		print ejecuta(command)
		print '#########################################################'
		DPUT = APP + '_' + VERSION+'-1ubuntu1_source.changes'
		command = 'dput ppa:atareao/test %s'%(os.path.join(PARENT_DIR,DPUT))
		print command
		print ejecuta(command)
		print '#########################################################'		
		for filetorm in glob.glob(os.path.join(PARENT_DIR,APP + '_' + VERSION+'*')):
			print 'removing %s'%filetorm
			os.remove(filetorm)	
